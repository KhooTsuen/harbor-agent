 ...
 */
const WORK_RULES = [
  '- **先看再改**。改文件前先 read_file 确认现在的样子，不要凭记忆改。\n- 确实是多步的活（要读好几个文件 / 改好几处 / 跑好几轮），**先给一个 ```plan 块**：\n  第一行写任务名（形如 `# 优化启动速度`，一句话说清在做什么），下面每行一条步骤。\n  做完一步就把那条标成 `[x]`，再重发一遍**完整**计划（界面照它显示进度）。\n  一两句就能答完的事不要写计划块。',
...
const BROWSER_GUIDE = [
  ...
  '- **密码框**：用户没明确说可以填时不要自己填。用户明确授权了（说了「可以填密码」、',
  '  或者直接把密码给了你），就正常调 `browse_type` —— 会有个确认弹窗让用户点一下，',
  '  那是他亲自授权。在弹窗里被拒绝就停下，不要再试。',
  '  填完之后**不要把密码重复写出来**，就说「已填入」。',
  '- 碰到登录墙、验证码、要用户本人确认的步骤，直接告诉用户，不要硬试。',
].join('\n')

/*
 * 提示词版本（②-1）。每轮会连同任务一起记进台账，诊断报告里看得到。
 *
 * **什么时候 +1** —— 任何可能改变模型行为的改动：
 *   · 加 / 删 / 改某一层的文案（WORK_RULES、SAFETY_GUIDE、MODE_GUIDE、各层模板…）
 *   · 改层的顺序（ORDER 变了，模型看到的重点就变了）
 *   · 加 / 删一层
 * 错别字、注释、纯重构（内容一字不变）不用动。
 *
