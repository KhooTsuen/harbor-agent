 ...
 * 还在长的尾巴。
 *
 * ★ **最后一行不参与解析**，直接当普通文字渲染 —— 这是文档要求的
 * 「流式时块不能频繁跳动」的正面修法。
 *
 * 为什么会跳：模型是一行一行吐出来的，而**写到一半的那行**往往是
 * 个残缺的标记：
 *
 *     `- `      空内容的列表项 → 解析器会把它当段落，于是 list → list,paragraph
 *     ```t      围栏还没写完 → 块类型在 paragraph / code 之间抻
 *     | A | B   表格还缺下一行的分割线 → 先当段落再变表格
 *
 * 实测（逐字喂，数类型签名变化次数）：
 *
 *     列表      不拆 5 次 → 拆了 0 次
 *     任务列表  不拆 3 次 → 拆了 0 次
 *     代码块    不拆 1 次 → 拆了 0 次
 *     表格      不拆 1 次 → 拆了 1 次（这个不可避免：第一行确实是段落）
 *
