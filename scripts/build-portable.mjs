/**
 * 打包成便携版 —— Electron 应用本质就是「一个 exe + 一个 resources 文件夹」，
...
/* credentials.json 是**这台机器的 API Key**：静默留着 = 每次打包都把它拷进便携版，再一路拷到测试副本 */
function quarantineCredentials(dataDir) {
  const cred = join(dataDir, 'credentials.json')
  if (!existsSync(cred)) return
  /* 时间戳格式刻意跟 core/backup.cjs 一致（本地时间 YYYYMMDD-HHMMSS）—— 同一个目录别混两套命名 */
  const d = new Date()
  const pad = (n) => String(n).padStart(2, '0')
  const at = `${d.getFullYear()}${pad(d.getMonth() + 1)}${pad(d.getDate())}-${pad(d.getHours())}${pad(d.getMinutes())}${pad(d.getSeconds())}`
  const dest = join(dataDir, 'backups', at, 'credentials.json')
  mkdirSync(dirname(dest), { recursive: true })
  renameSync(cred, dest)
  console.log(`\n  ⚠️⚠️⚠️  data/credentials.json（API Key）已隔离到 data/backups/${at}/`)
  console.log('         —— 它不该跟着便携版走（没删，要就自己拷回去）\n')
}
...
function copyRuntime() {
  ...
  /*
   * Electron 默认带 50 多种 Chromium 界面语言，我们界面只有中文。
   * 保留中/英文（Chromium 的安全回退），其余删掉 —— 不影响模型、文件、终端、webview。
   */
  ...
  /* 换 exe 图标 —— 用 Win32 UpdateResource 直接覆盖，不用 rcedit（rcedit 会先「删除旧图标资源」，
     在某些机器上返回 1359 并弄坏句柄） */
...
function copyApp() {
  ...
  /* 关键：Electron 靠这个文件认「这里是个应用」，缺了窗口打不开、**没有任何报错** */
...
/** 读某个 package.json 的 version；读不到（没装 / 坏了）返回 null —— 清单不该让打包失败 */
function readVersion(...seg) {
  try {
    return JSON.parse(readFileSync(join(...seg), 'utf8')).version ?? null
  } catch {
    return null
  }
}

/** 读 git commit；读不到（没 init git / 没装 git）返回 null，**不因此让打包失败** */
function readGitCommit() {
  try {
    const r = spawnSync('git', ['rev-parse', 'HEAD'], { cwd: ROOT, encoding: 'utf8', timeout: 5000 })
    const out = String(r.stdout ?? '').trim()
    return r.status === 0 && /^[0-9a-f]{7,40}$/i.test(out) ? out : null
  } catch {
    return null
  }
}
...
/**
 * 写「这份便携版是怎么来的」（构建产物，不进仓库）：光问版本号答不出 Electron / node-pty / commit。
 * node-pty 1.x 是 N-API 预编译包，**没有数字 ABI**：有本平台产物才记 'napi'。
 */
function writeBuildInfo() {
  const at = `${process.platform}-${process.arch}`
  const info = {
    version: readVersion(ROOT, 'package.json'),
    platform: process.platform,
    arch: process.arch,
    electronVersion: readVersion(ROOT, 'node_modules', 'electron', 'package.json'),
    nodePtyAbi: existsSync(join(ROOT, 'node_modules', 'node-pty', 'prebuilds', at)) ? 'napi' : null,
    buildTime: new Date().toISOString(),
    gitCommit: readGitCommit(),
  }
  const file = join(APP_OUT, 'build-info.json')
...
function verify() {
  const must = [
    ...
    [join(APP_OUT, 'build-info.json'), '构建清单'],
  ]
