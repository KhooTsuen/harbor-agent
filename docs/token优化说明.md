...
- **预热目标口径（2026-09-26 修订）**：验收目标为「**预热静态段复用率 >80%**」。预热只冻住静态段，而首请求还要带会话/任务状态与用户消息——整体命中 >50% 在数学上不可达。实测：静态段复用 1280/1588 = **80.6%，达标**（整体首请求 33.2% 保留为参考值；修正说明见测试记录 §4.6 与问题记录 TOK-P2-005）。
