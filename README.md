...
| MCP 外部工具 | 🟡 可用 | 只实现 stdio；按**不可信**处理（环境白名单 + 返回值标注） |
| 本地插件 | 🟡 可用 | 自己放进 `data/plugins/`；支持热插拔 |
| **跨仓库操作** | 🔴 实验性 | 没有多项目隔离模型，别指望 |
| **数据库迁移 / 碰生产环境** | 🔴 实验性 | 风险分级会拦 critical，但**别拿它碰生产** |
...
| Linux | ✅ 理论可以 | ❌ **没验过** | 无 keyring 时密钥**退化为明文**，界面会明确标出来 |
